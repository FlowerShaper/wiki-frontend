...
<template>
    <Transition name="search">
        <div
            class="pointer-events-auto fixed size-full max-h-screen overflow-y-scroll bg-background-1/75"
        ...
        >
            <div class="mx-auto flex w-full flex-col gap-3 px-4 pt-24 lg:w-search" @click="$event!.stopPropagation()">
                <input
                    ref="input"
                    @input="InputUpdated"
                    class="h-12 rounded-lg border-2 border-background-3 bg-background-2 px-3 placeholder:text-text/50 focus:border-primary focus:outline-none"
                    type="text"
                    :value="query"
                ...
                <NuxtLink
                    :to="article.url"
                    class="overlap-grid h-16 w-full overflow-hidden rounded-lg bg-background-2"
                    v-for="article in results"
                    @click="state.overlays.search = false"
                >
                    <img class="w-1/3! object-cover" :src="api.ResolveAsset(article.meta.image)" v-if="article.meta.image" />
                    <div class="bg-linear-to-r from-background-2/80 to-background-2 to-20% hover:opacity-80"></div>
