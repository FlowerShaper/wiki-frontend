...
import type { ParsedMarkdown } from '#imports';
...
if (article) {
    parsed = await markdown.Parse(article.content);
    ...
    <PageBase>
        <div class="flex flex-col gap-9" v-if="article && content">
            <div class="flex w-full flex-row">
                ...
                <div class="mx-6 hidden w-1 rounded bg-background-2 md:block"></div>
                <div class="flex min-w-0 max-w-full flex-1 flex-col gap-4">
                    ...
                    <MarkdownDocument class="md-content" :components="markdown.Components" :value="parsed" />
                </div>
            </div>
